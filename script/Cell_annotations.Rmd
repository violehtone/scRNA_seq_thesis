...
colpalette <- c('primary_data_cells' ="#DC143C", ##009900
                'other_cell_types' = "#A9A9A9") ##404040

# Draw trajectory plot
traj.plot.d5 <- draw_trajectory_plot(space = space.deMicheli, 
...
df.dellOrso.f <- melt(t(tab.dellOrso.f))
colnames(df.dellOrso.f) <- c("cell_type", "sample", "count")
...
# df.dellOrso.missing.rows.f <- data.frame("cell_type" = rep("Primary_MB", 5),
#                                        "sample" = c("control", "cap50", "cap50_r4h", "cap50_r8h", "cap50_r16h"),
#                                        "count" = rep(0, 5))

df.deMicheli <- rbind(df.deMicheli, df.deMicheli.missing.rows)
df.dellOrso <- rbind(df.dellOrso, df.dellOrso.missing.rows)
df.dellOrso.f <- rbind(df.dellOrso.f, df.dellOrso.missing.rows.f)
...
# Dell Orso balloon plot (without PMs)
balloon.dellOrso.f <- ggballoonplot(df.dellOrso.f, x = "sample", y = "cell_type", size = "count",
              fill = "count", ggtheme = theme_bw()) +
  scale_fill_viridis_c(option = "C") +
  ggtitle("Dell'Orso et al. reference dataset: Mapping of primary data cells shown in a balloonplot")
...
## 6 Save cell annotation plots as R objects
```{r save-plots-as-r-objects, eval=FALSE}
# Balloon plots
saveRDS(balloon.deMicheli + ggtitle("", subtitle = ""), file = "./../saved/R_objects/plot_balloon_deMicheli")
saveRDS(balloon.dellOrso + ggtitle("", subtitle = ""), file = "./../saved/R_objects/plot_balloon_dellOrso")
saveRDS(balloon.dellOrso.f + ggtitle("", subtitle = ""), file = "./../saved/R_objects/plot_balloon_dellOrso_f")
saveRDS(balloon.segments + ggtitle("", subtitle = ""), file = "./../saved/R_objects/plot_balloon_segments")
saveRDS(balloon.segments.d5 + ggtitle("", subtitle = ""), file = "./../saved/R_objects/plot_balloon_segments_d5")

# Trajectories
saveRDS(traj.plot.d5 + ggtitle("", subtitle = ""), file = "./../saved/R_objects/plot_cell_annotation_trajectory_d5")
```
