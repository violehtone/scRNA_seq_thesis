...
```{r de-genes-primary-data-samples}
...
write.csv(control.markers, file = "./../saved/control_marker_genes.csv")
...
**Markers by cluster**
```{r de-genes-primary-data-clusters}
# Find markers by sample
...
for(cluster in unique(denoised.sce$cluster)) {
  ...
  # Save marker information to .csv file
  fpath <- paste0("./../saved/marker_genes_cluster_", cluster, ".csv")
...
```{r de-genes-primary-data-clusters-cc}
# Find markers by sample
...
for(cluster in unique(denoised.sce$cluster.cc)) {
  ...
  # Save marker information to .csv file
  fpath <- paste0("./../saved/marker_genes_cluster_", cluster, "_cc.csv")
  write.csv(markers, file = fpath)
  # Subset top 6 genes (ranked by p -value) from each pairwise comparison involving the cluster x
  best.set <- markers[markers$Top <= 6,]
  # Extract the marker effect sizes
...
# Save marker information to .csv file
write.csv(control.markers.wmw, file = "./../saved/control_marker_genes_wmw.csv")
...
for(cluster in unique(denoised.sce$cluster)) {
  ...
  # Save marker information to .csv file
  fpath <- paste0("./../saved/marker_genes_cluster_", cluster, "_wmw.csv")
...
for(cluster in unique(denoised.sce$cluster.cc)) {
  ...
  # Save marker information to .csv file
  fpath <- paste0("./../saved/marker_genes_cluster_", cluster, "cc_wmw.csv")
  write.csv(markers.clust.wmw, file = fpath)
  # Subset top 6 genes (ranked by p -value) from each pairwise comparison involving the cluster x
  best.set <- markers.clust.wmw[markers.clust.wmw$Top <= 6,]
  # Extract the area under curves
