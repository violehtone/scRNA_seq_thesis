...
## 1. Introduction
The aim of this script is to load and process 3 scRNA-seq datasets. One of the datasets is our primary data, which is not publicly available and the two others are reference datasets downloaded from Gene Expression Omnibus (GEO). The instructions for downloading the reference datasets are shown below:

  - De Micheli et al. (2020): GSE143437 data set (https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE143437)
  - Dell'Orso et al. (2019): GSE126834 data set (https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE126834)

This script requires a correct folder structure (shown on README.md) for it to function. Also, the evaluation of majority of the code chunks is se to FALSE to speeden up the computation times. Instead of computation, many parts of the script use pre-calculated R objects.

**Note**: In case you are running this script for the first time, please compute the required R objects by running the code chunks needed to produce them.
