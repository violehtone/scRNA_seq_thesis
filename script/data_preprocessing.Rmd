...
# Filter only relevant samples for building the MuSC trajectory
...
plotReducedDim(denoised.sce, colour_by = "sample", dimred = "force") + ggtitle("Primary data clustering: force-directed layout")
plotReducedDim(denoised.sce, colour_by = "sample", dimred = "PCA") + ggtitle("Primary data clustering: PCA")

denoised.sce <- runUMAP(denoised.sce, dimred = "PCA", n_dimred = dimensions)
plotReducedDim(denoised.sce, colour_by = "sample", dimred = "UMAP") + ggtitle("Primary data clustering: UMAP")
plotReducedDim(denoised.sce, colour_by = "cluster", dimred = "UMAP") + ggtitle("Primary data clustering: UMAP with clusters")

denoised.sce <- runTSNE(denoised.sce, dimred = "PCA")
plotTSNE(denoised.sce, colour_by = "sample") + ggtitle("Primary data clustering: TSNE")
...
# Plot results
plotReducedDim(denoised.sce.cc, colour_by = "sample", dimred = "force") + ggtitle("Primary data clustering (Seurat cc correction): force-directed layout")
plotReducedDim(denoised.sce.cc, colour_by = "sample", dimred = "PCA") + ggtitle("Primary data clustering (Seurat cc correction): PCA")

denoised.sce.cc <- runUMAP(denoised.sce.cc, dimred = "PCA", n_dimred = dimensions.cc)
plotReducedDim(denoised.sce.cc, colour_by = "sample", dimred = "UMAP") + ggtitle("Primary data clustering (Seurat cc correction): UMAP")
plotReducedDim(denoised.sce.cc, colour_by = "cluster", dimred = "UMAP") + ggtitle("Primary data clustering (Seurat cc correction): UMAP with clusters")

denoised.sce.cc <- runTSNE(denoised.sce.cc, dimred = "PCA")
plotTSNE(denoised.sce.cc, colour_by = "sample") + ggtitle("Primary data clustering (Seurat cc correction): TSNE")

#saveRDS(denoised.sce.cc, file = "./../saved/denoised_sce_cc_clust")
```

```{r primary-cc-data-cluster-comparison}
# Compare clusters vs. samples
gridExtra::grid.arrange(
  plotReducedDim(denoised.sce.cc, colour_by = "sample", dimred = "UMAP", text_by = "cluster", text_size = 3) + ggtitle("Primary data (cc) UMAP plot: samples"),
  plotReducedDim(denoised.sce.cc, colour_by = "cluster", dimred = "UMAP", text_by = "sample", text_size = 3) + ggtitle("Primary data (cc) UMAP plot: clusters"),
  ncol = 1,
  nrow = 2
)
...
```{r primary-cc-data-cluster-stability, eval= FALSE}
# Evaluating cluster stability
myClusterFUN <- function(x) {
  d <- ncol(reducedDim(x))
  g <- buildSNNGraph(x, d=d, assay.type = "counts")
  igraph::cluster_louvain(g)$membership
}

originals <- myClusterFUN(denoised.sce.cc)

# NOTE: Commented because bootstrapCluster() is computationally very heavy and so we use here pre-saved object
#set.seed(123)
#coassign <- bootstrapCluster(denoised.sce.cc, FUN = myClusterFUN, clusters = originals)
coassign <- readRDS("./../saved/coassign")

dim(coassign)

# Heatmap of coassignment probabilities from bootstrapping of graph-based clustering in the cc-corrected primary dataset.
# Each row and column represents an original cluster and each entry is colored according to the value of the coassignment probability between that pair of clusters.
