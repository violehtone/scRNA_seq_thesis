...
### 10.4 Annotation of cell cycle phases to reference datasets
```{r ref-cellcycle-annotation}
...
# Diagnose if cells separate by their phase
set.seed(123)
uncorrected.deMicheli.cc <- runPCA(sce.deMicheli.cc, subset_row = c(s.genes.mouse, g2m.genes.mouse), BSPARAM = BiocSingular::RandomParam())
uncorrected.deMicheli.cc <- runTSNE(uncorrected.deMicheli.cc, dimred = "PCA")
plotTSNE(uncorrected.deMicheli.cc, colour_by = "Phase") + ggtitle("De Micheli et al.: TSNE plot for Diagnosing cell cycle effects")

set.seed(123)
sce.dellOrso.cc <- runPCA(sce.dellOrso.cc, subset_row = c(s.genes.mouse, g2m.genes.mouse), BSPARAM = BiocSingular::RandomParam())
sce.dellOrso.cc <- runTSNE(sce.dellOrso.cc, dimred = "PCA")
plotTSNE(sce.dellOrso.cc, colour_by = "Phase") + ggtitle("DellOrso et al.: TSNE plot for Diagnosing cell cycle effects")

# Plot cc phases for the datasets
phases.deMicheli <- as.data.frame.matrix(table(sce.deMicheli.cc$Phase, sce.deMicheli.cc$injury))
phases.dellOrso <- as.data.frame.matrix(table(sce.dellOrso.cc$Phase, sce.dellOrso.cc$sample))

par(mfrow=c(1,3))
pie(phases.deMicheli$`Day 0`, labels = rownames(phases.deMicheli), main = "Day 0")
pie(phases.deMicheli$`Day 5`, labels = rownames(phases.deMicheli), main = "Day 5")
pie(phases.deMicheli$`Day 7`, labels = rownames(phases.deMicheli), main = "Day 7")

par(mfrow=c(2,3))
pie(phases.dellOrso$homeostatic_MuSCs_rep1, labels = rownames(phases.dellOrso), main = "homeostatic_MuSCs_rep1")
pie(phases.dellOrso$homeostatic_MuSCs_rep2, labels = rownames(phases.dellOrso), main = "homeostatic_MuSCs_rep2")
pie(phases.dellOrso$inj_60h_MuSCs_rep1, labels = rownames(phases.dellOrso), main = "inj_60h_MuSCs_rep1")
pie(phases.dellOrso$inj_60h_MuSCs_rep2, labels = rownames(phases.dellOrso), main = "inj_60h_MuSCs_rep2")
pie(phases.dellOrso$Primary_MB, labels = rownames(phases.dellOrso), main = "Primary_MB")
```
...
sce.deMicheli.cc <- runPCA(sce.deMicheli.cc, subset_row = hvgs.deMicheli)
plotReducedDim(sce.deMicheli.cc, dimred = "PCA", colour_by = "injury")

# Compute the variance explained by each PC
percent.var.deMicheli <- attr(reducedDim(sce.deMicheli.cc), "percentVar")
...
# Remove PCs corresponding to technical noise
set.seed(123)
denoised.sce.deMicheli <- denoisePCA(sce.deMicheli.cc,
...
# Perform dimension reduction
sce.dellOrso.cc <- runPCA(sce.dellOrso.cc, subset_row = hvgs.dellOrso)
plotReducedDim(sce.dellOrso.cc, dimred = "PCA", colour_by = "sample")

# Compute the variance explained by each PC
percent.var.dellOrso <- attr(reducedDim(sce.dellOrso.cc), "percentVar")
...
# Remove PCs corresponding to technical noise
set.seed(123)
denoised.sce.dellOrso <- denoisePCA(sce.dellOrso.cc,
...
# Perform clustering with k=180
dimensions <- ncol(reducedDim(denoised.sce))

snng <- buildSNNGraph(denoised.sce, d = dimensions, k = 180)
...
# Plot reduced dims
plotReducedDim(denoised.sce, colour_by = "sample", dimred = "force") + ggtitle("Primary data: force-directed layout (k=180)") + theme(plot.title = element_text(size = 16, face = "bold"))

plotReducedDim(denoised.sce, colour_by = "sample", dimred = "PCA") + ggtitle("Primary data: PCA (k=180)") + theme(plot.title = element_text(size = 16, face = "bold"))
...
plotTSNE(denoised.sce, colour_by = "sample") + ggtitle("Primary data clustering: TSNE (k=180)") + theme(plot.title = element_text(size = 16, face = "bold"))
...
snng.cc <- buildSNNGraph(denoised.sce.cc, d = dimensions.cc, assay.type = "counts", k = 180)
...
print("Modularity of the graph: ", mod.cc)
pheatmap(log2(ratio.cc + 1), cluster_cols = FALSE, cluster_rows = FALSE, color=colorRampPalette(c("white", "blue"))(100), main = "Primary data cell cycle corrected, k = 180")

# Perform silhouette analysis
dist.data.cc <- dist(reducedDim(denoised.sce.cc, "PCA"))
sil.cc <- silhouette(clust.cc, dist = dist.data.cc)
plot(sil.cc)

# Silhouette with subset of cells
denoised.sce.cc.f <- denoised.sce.cc[, seq(1, 21808, by = 80)]
clust.cc.f <- clust.cc[seq(1, 21808, by = 80)]
dist.data.cc.f <- dist(reducedDim(denoised.sce.cc.f, "PCA"))
sil.cc.f <- silhouette(clust.cc.f, dist = dist.data.cc.f)
plot(sil.cc.f)

# Plot reduced dims
plotReducedDim(denoised.sce.cc, colour_by = "sample", dimred = "force") + ggtitle("Primary data (cc corrected) clustering: force-directed layout (k=10)") + theme(plot.title = element_text(size = 16, face = "bold"))

plotReducedDim(denoised.sce.cc, colour_by = "sample", dimred = "PCA") + ggtitle("Primary data (cc corrected) clustering: PCA (k=180)") + theme(plot.title = element_text(size = 16, face = "bold"))

plotReducedDim(denoised.sce.cc, colour_by = "sample", dimred = "UMAP") + ggtitle("Primary data (cc corrected): UMAP with clusters (k=180)") + theme(plot.title = element_text(size = 16, face = "bold"))

plotReducedDim(denoised.sce.cc, colour_by = "cluster", dimred = "UMAP") + ggtitle("Primary data (cc corrected) with clusters: UMAP with clusters (k=180)") + theme(plot.title = element_text(size = 16, face = "bold"))

plotTSNE(denoised.sce.cc, colour_by = "sample") + ggtitle("Primary data (cc corrected) clustering: TSNE (k=180)") + theme(plot.title = element_text(size = 16, face = "bold"))
...
**This code chunk should maybe be deleted as it is not used anymore(?)**
```{r primary-data-cc2-clustering, eval=FALSE}
# Perform clustering
...
**This code chunk should maybe be deleted as it is not used anymore(?)**
```{r deMicheli-clustering, eval=FALSE}
# Perform clustering
dimensions.deMicheli <- ncol(reducedDim(denoised.sce.deMicheli))
snng.deMicheli <- buildSNNGraph(denoised.sce.deMicheli, d = dimensions.deMicheli, k = 50)
...
**This code chunk should maybe be deleted as it is not used anymore(?)**
```{r dellOrso-clustering, eval=FALSE}
# Perform clustering
dimensions.dellOrso <- ncol(reducedDim(denoised.sce.dellOrso))
snng.dellOrso <- buildSNNGraph(denoised.sce.dellOrso, d = dimensions.dellOrso, k = 50)
