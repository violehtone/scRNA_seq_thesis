...
Note: the above data loading and sce object creation are not executed because the sce files of the 3 datasets are directly loaded from the files shown below.
...
```
 - As can be seen from the plot, the clusters seem to separate mostly by the cell types
 - In the Day 7 sample, however, the samples seem to create separate clusters which cannot be explained by the differences in cell types.
...
# Perform clustering with 3 different values for k (5, 10, 50)
dimensions <- ncol(reducedDim(denoised.sce))

# k=5
denoised.sce.5 <- denoised.sce
snng.5 <- buildSNNGraph(denoised.sce, d = dimensions, k=5)
clust.5 <- igraph::cluster_walktrap(snng.5)$membership
table(clust.5)
denoised.sce.5$cluster <- factor(clust.5)

set.seed(123)
reducedDim(denoised.sce.5, "force") <- igraph::layout_with_fr(snng.5)
denoised.sce.5 <- runUMAP(denoised.sce.5, dimred = "PCA", n_dimred = dimensions)
denoised.sce.5 <- runTSNE(denoised.sce.5, dimred = "PCA")

mod.5 <- modularity(snng.5, clust.5)
ratio.5 <- clusterModularity(snng.5, clust.5, as.ratio = TRUE)

# k=10
denoised.sce.10 <- denoised.sce
snng.10 <- buildSNNGraph(denoised.sce.10, d = dimensions)
...
table(clust.50)
denoised.sce.50$cluster <- factor(clust.50)

set.seed(123)
reducedDim(denoised.sce.50, "force") <- igraph::layout_with_fr(snng.50)
denoised.sce.50 <- runUMAP(denoised.sce.10, dimred = "PCA", n_dimred = dimensions)
denoised.sce.50 <- runTSNE(denoised.sce.10, dimred = "PCA")

mod.50 <- modularity(snng.50, clust.50)
ratio.50 <- clusterModularity(snng.50, clust.50, as.ratio = TRUE)

# Compare clustering results
tab.5.10 <- table(paste("k=5", clust.5), paste("k=10", clust.10))
ivw.5.10 <- pheatmap(log10(tab.5.10 + 10), main = "k=5 vs. k=10", color=viridis::viridis(100), silent = TRUE)

tab.50.10 <- table(paste("k=50", clust.50), paste("k=10", clust.10))
ivw.50.10 <- pheatmap(log10(tab.50.10 + 10), main = "k=50 vs. k=10", color=viridis::viridis(100), silent = TRUE)

tab.50.5 <- table(paste("k=50", clust.50), paste("k=5", clust.5))
ivw.50.5 <- pheatmap(log10(tab.50.5 + 10), main = "k=50 vs. k=5", color=viridis::viridis(100), silent = TRUE)

gridExtra::grid.arrange(ivw.5.10[[4]], ivw.50.10[[4]], ivw.50.5[[4]], top = "Primary data: Comparison of clustering strategies with different k")

# Plot Force-, PCA-, tSNE-, and UMAP plots
p.force.5 <-   plotReducedDim(denoised.sce.5, colour_by = "sample", dimred = "force") + ggtitle("Primary data clustering: force-directed layout (k=5)")
p.pca.5 <- plotReducedDim(denoised.sce.5, colour_by = "sample", dimred = "PCA") + ggtitle("Primary data clustering: PCA (k=5)")
p.umap.5 <- plotReducedDim(denoised.sce.5, colour_by = "cluster", dimred = "UMAP") + ggtitle("Primary data clustering: UMAP with clusters (k=5)")
p.tsne.5 <- plotTSNE(denoised.sce.5, colour_by = "sample") + ggtitle("Primary data clustering: TSNE (k=5)")

p.force.10 <- plotReducedDim(denoised.sce.10, colour_by = "sample", dimred = "force") + ggtitle("Primary data clustering: force-directed layout (k=10)")
p.pca.10 <- plotReducedDim(denoised.sce.10, colour_by = "sample", dimred = "PCA") + ggtitle("Primary data clustering: PCA (k=10)")
p.umap.10 <- plotReducedDim(denoised.sce.10, colour_by = "cluster", dimred = "UMAP") + ggtitle("Primary data clustering: UMAP with clusters (k=10)")
p.tsne.10 <- plotTSNE(denoised.sce.10, colour_by = "sample") + ggtitle("Primary data clustering: TSNE (k=10)")

p.force.50 <-   plotReducedDim(denoised.sce.50, colour_by = "sample", dimred = "force") + ggtitle("Primary data clustering: force-directed layout (k=50)")
p.pca.50 <- plotReducedDim(denoised.sce.50, colour_by = "sample", dimred = "PCA") + ggtitle("Primary data clustering: PCA (k=50)")
p.umap.50 <- plotReducedDim(denoised.sce.50, colour_by = "cluster", dimred = "UMAP") + ggtitle("Primary data clustering: UMAP with clusters (k=50)")
p.tsne.50 <- plotTSNE(denoised.sce.50, colour_by = "sample") + ggtitle("Primary data clustering: TSNE (k=50)")

gridExtra::grid.arrange(p.force.5, p.force.10, p.force.50, top = "Primary data: force-directed layout plot for k-values 5,10,50")
gridExtra::grid.arrange(p.pca.5, p.pca.10, p.pca.50, top = "Primary data: PCA plot for k-values 5,10,50")
gridExtra::grid.arrange(p.umap.5, p.umap.10, p.umap.50, top = "Primary data: UMAP plot for k-values 5,10,50")
gridExtra::grid.arrange(p.tsne.5, p.tsne.10, p.tsne.50, top = "Primary data: tSNE plot for k-values 5,10,50")

# Plot heatmap of modularities for each k
mod.df <- DataFrame("k=5" = mod.5, "k=10" = mod.10, "k=50" = mod.50)
mod.df

pheatmap(log2(ratio.5 + 1), cluster_cols = FALSE, cluster_rows = FALSE, color=colorRampPalette(c("white", "blue"))(100), main = "k = 5")
pheatmap(log2(ratio.10 + 1), cluster_cols = FALSE, cluster_rows = FALSE, color=colorRampPalette(c("white", "blue"))(100), main = "k = 10")
pheatmap(log2(ratio.50 + 1), cluster_cols = FALSE, cluster_rows = FALSE, color=colorRampPalette(c("white", "blue"))(100), main = "k = 50")

# Set k=5 to be used as k-value
denoised.sce <- denoised.sce.5
...
# k=5
denoised.sce.cc.5 <- denoised.sce.cc
snng.cc.5 <- buildSNNGraph(denoised.sce.cc, d = dimensions.cc, k=5)
clust.cc.5 <- igraph::cluster_walktrap(snng.cc.5)$membership
table(clust.cc.5)
denoised.sce.cc.5$cluster <- factor(clust.cc.5)

set.seed(123)
reducedDim(denoised.sce.cc.5, "force") <- igraph::layout_with_fr(snng.cc.5)
denoised.sce.cc.5 <- runUMAP(denoised.sce.cc.5, dimred = "PCA", n_dimred = dimensions.cc)
denoised.sce.cc.5 <- runTSNE(denoised.sce.cc.5, dimred = "PCA")

mod.cc.5 <- modularity(snng.cc.5, clust.cc.5)
ratio.cc.5 <- clusterModularity(snng.cc.5, clust.cc.5, as.ratio = TRUE)

# k=10
denoised.sce.cc.10 <- denoised.sce.cc
snng.cc.10 <- buildSNNGraph(denoised.sce.cc.10, d = dimensions.cc)
...
snng.cc.50 <- buildSNNGraph(denoised.sce.cc.50, d = dimensions.cc)
clust.cc.50 <- igraph::cluster_walktrap(snng.cc.50)$membership
table(clust.cc.50)
denoised.sce.cc.50$cluster <- factor(clust.cc.50)

set.seed(123)
reducedDim(denoised.sce.cc.50, "force") <- igraph::layout_with_fr(snng.cc.50)
denoised.sce.cc.50 <- runUMAP(denoised.sce.cc.50, dimred = "PCA", n_dimred = dimensions.cc)
denoised.sce.cc.50 <- runTSNE(denoised.sce.cc.50, dimred = "PCA")

mod.cc.50 <- modularity(snng.cc.50, clust.cc.50)
ratio.cc.50 <- clusterModularity(snng.cc.50, clust.cc.50, as.ratio = TRUE)

# Compare clustering results
tab.5.10 <- table(paste("k=5", clust.cc.5), paste("k=10", clust.cc.10))
ivw.5.10 <- pheatmap(log10(tab.5.10 + 10), main = "k=5 vs. k=10", color=viridis::viridis(100), silent = TRUE)

tab.50.10 <- table(paste("k=50", clust.cc.50), paste("k=10", clust.cc.10))
ivw.50.10 <- pheatmap(log10(tab.50.10 + 10), main = "k=50 vs. k=10", color=viridis::viridis(100), silent = TRUE)

tab.50.5 <- table(paste("k=50", clust.cc.50), paste("k=5", clust.cc.5))
ivw.50.5 <- pheatmap(log10(tab.50.5 + 10), main = "k=50 vs. k=5", color=viridis::viridis(100), silent = TRUE)

gridExtra::grid.arrange(ivw.5.10[[4]], ivw.50.10[[4]], ivw.50.5[[4]], top = "Primary data (cc corrected): Comparison of clustering strategies with different k")

# Plot Force-, PCA-, tSNE-, and UMAP plots
p.force.5 <- plotReducedDim(denoised.sce.cc.5, colour_by = "sample", dimred = "force") + ggtitle("Primary data (cc corrected) clustering: force-directed layout (k=5)")
p.pca.5 <- plotReducedDim(denoised.sce.cc.5, colour_by = "sample", dimred = "PCA") + ggtitle("Primary data (cc corrected) clustering: PCA (k=5)")
p.umap.5 <- plotReducedDim(denoised.sce.cc.5, colour_by = "cluster", dimred = "UMAP") + ggtitle("Primary data (cc corrected) clustering: UMAP with clusters (k=5)")
p.tsne.5 <- plotTSNE(denoised.sce.cc.5, colour_by = "sample") + ggtitle("Primary data (cc corrected) clustering: TSNE (k=5)")

p.force.10 <-   plotReducedDim(denoised.sce.cc.10, colour_by = "sample", dimred = "force") + ggtitle("Primary data (cc corrected) clustering: force-directed layout (k=10)")
p.pca.10 <- plotReducedDim(denoised.sce.cc.10, colour_by = "sample", dimred = "PCA") + ggtitle("Primary data (cc corrected) clustering: PCA (k=10)")
p.umap.10 <- plotReducedDim(denoised.sce.cc.10, colour_by = "cluster", dimred = "UMAP") + ggtitle("Primary data (cc corrected) clustering: UMAP with clusters (k=10)")
p.tsne.10 <- plotTSNE(denoised.sce.cc.10, colour_by = "sample") + ggtitle("Primary data (cc corrected) clustering: TSNE (k=10)")

p.force.50 <-   plotReducedDim(denoised.sce.cc.50, colour_by = "sample", dimred = "force") + ggtitle("Primary data (cc corrected) clustering: force-directed layout (k=50)")
p.pca.50 <- plotReducedDim(denoised.sce.cc.50, colour_by = "sample", dimred = "PCA") + ggtitle("Primary data (cc corrected) clustering: PCA (k=50)")
p.umap.50 <- plotReducedDim(denoised.sce.cc.50, colour_by = "cluster", dimred = "UMAP") + ggtitle("Primary data (cc corrected) clustering: UMAP with clusters (k=50)")
p.tsne.50 <- plotTSNE(denoised.sce.cc.50, colour_by = "sample") + ggtitle("Primary data (cc corrected) clustering: TSNE (k=50)")

gridExtra::grid.arrange(p.force.5, p.force.10, p.force.50, top = "Primary data (cc corrected): force-directed layout plot for k-values 5,10,50")
gridExtra::grid.arrange(p.pca.5, p.pca.10, p.pca.50, top = "Primary data (cc corrected): PCA plot for k-values 5,10,50")
gridExtra::grid.arrange(p.umap.5, p.umap.10, p.umap.50, top = "Primary data (cc corrected): UMAP plot for k-values 5,10,50")
gridExtra::grid.arrange(p.tsne.5, p.tsne.10, p.tsne.50, top = "Primary data (cc corrected): tSNE plot for k-values 5,10,50")

# Plot heatmap of modularities for each k
mod.df <- DataFrame("k=5" = mod.cc.5, "k=10" = mod.cc.10, "k=50" = mod.cc.50)
mod.df

pheatmap(log2(ratio.cc.5 + 1), cluster_cols = FALSE, cluster_rows = FALSE, color=colorRampPalette(c("white", "blue"))(100), main = "k = 5")
pheatmap(log2(ratio.cc.10 + 1), cluster_cols = FALSE, cluster_rows = FALSE, color=colorRampPalette(c("white", "blue"))(100), main = "k = 10")
pheatmap(log2(ratio.cc.50 + 1), cluster_cols = FALSE, cluster_rows = FALSE, color=colorRampPalette(c("white", "blue"))(100), main = "k = 50")

# Set k=5 to be used as k-value
denoised.sce.cc <- denoised.sce.cc.5
...
#### 12.2.1 Primary data (Seurat cell cycle correction): Subclustering and marker genes


```{r primary-cc-data-cluster-comparison}
# Inspect clusters in each sample
table(denoised.sce.cc$sample, denoised.sce.cc$cluster)

# Find marker genes between clusters
markers <- findMarkers(denoised.sce.cc, groups = denoised.sce.cc$cluster, assay.type = "counts", pval.type="any", lfc = 1)

# Plot the marker genes in a heatmap
for(i in 1:length(markers)) {
  chosen <- markers[[i]]
  logFCs <- getMarkerEffects(chosen[chosen$Top <= 5, ])
  pheatmap(logFCs, breaks=seq(-5, 5, length.out=101), main = paste("Cluster: ", i))
}

# Subclustering
subclust <- quickSubCluster(denoised.sce.cc, groups = denoised.sce.cc$sample, assay.type = "counts", normalize = FALSE)

plotReducedDim(subclust[["control"]], colour_by = "subcluster", dimred = "UMAP") + ggtitle("Primary data (cc), Cap50 sample - UMAP plot: subclusters")
plotReducedDim(subclust[["cap50"]], colour_by = "subcluster", dimred = "UMAP") + ggtitle("Primary data (cc), Cap50 sample - UMAP plot: subclusters")
plotReducedDim(subclust[["cap50_r4h"]], colour_by = "subcluster", dimred = "UMAP") + ggtitle("Primary data (cc), R4H sample - UMAP plot: subclusters")
plotReducedDim(subclust[["cap50_r8h"]], colour_by = "subcluster", dimred = "UMAP") + ggtitle("Primary data (cc), R8H sample - UMAP plot: subclusters")
plotReducedDim(subclust[["cap50_r16h"]], colour_by = "subcluster", dimred = "UMAP") + ggtitle("Primary data (cc), R16H sample - UMAP plot: subclusters")

# Find markers within subclusters
markers.control <- findMarkers(subclust[["control"]], groups = subclust[["control"]]$subcluster, assay.type = "counts", pval.type = "any", lfc = 1)
markers.cap50 <- findMarkers(subclust[["cap50"]], groups = subclust[["cap50"]]$subcluster, assay.type = "counts", pval.type = "any", lfc = 1)
markers.cap50_r4h <- findMarkers(subclust[["cap50_r4h"]], groups = subclust[["cap50_r4h"]]$subcluster, assay.type = "counts", pval.type = "any", lfc = 1)
markers.cap50_r8h <- findMarkers(subclust[["cap50_r8h"]], groups = subclust[["cap50_r8h"]]$subcluster, assay.type = "counts", pval.type = "any", lfc = 1)
markers.cap50_r16h <- findMarkers(subclust[["cap50_r16h"]], groups = subclust[["cap50_r16h"]]$subcluster, assay.type = "counts", pval.type = "any", lfc = 1)

# Plot marker genes for samples
for(i in 1:length(markers.cap50)) {
  chosen <- markers.cap50[[i]]
  logFCs <- getMarkerEffects(chosen[chosen$Top <= 5, ])
  pheatmap(logFCs, breaks=seq(-5, 5, length.out=101), main = paste("Cap50 Subcluster:", i))
}

for(i in 1:length(markers.cap50_r4h)) {
  chosen <- markers.cap50_r4h[[i]]
  logFCs <- getMarkerEffects(chosen[chosen$Top <= 5, ])
  pheatmap(logFCs, breaks=seq(-5, 5, length.out=101), main = paste("Cap50_R4H Subcluster:", i))
}

for(i in 1:length(markers.cap50_r8h)) {
  chosen <- markers.cap50_r8h[[i]]
  logFCs <- getMarkerEffects(chosen[chosen$Top <= 5, ])
  pheatmap(logFCs, breaks=seq(-5, 5, length.out=101), main = paste("Cap50_R8H Subcluster:", i))
}

for(i in 1:length(markers.cap50_r16h)) {
  chosen <- markers.cap50_r16h[[i]]
  logFCs <- getMarkerEffects(chosen[chosen$Top <= 5, ])
  pheatmap(logFCs, breaks=seq(-5, 5, length.out=101), main = paste("Cap50_R16H Subcluster:", i))
}
