...
p.primary <- ggplot(df.modules.primary.separated, aes(x = as.factor(Scorpius_module), y = frequency)) +
  ...
  geom_text(aes(label=ifelse(frequency>=4, KEGG_module, "")), position = position_dodge(width=0.9), vjust=-0.5)
...
#### 8.8 Create mapping objects between genes, Scorpius modules, and metabolic pathways
```{r}
if(!exists("modules.primary.m")){modules.primary.m <- readRDS("./../saved/modules_primary_m_kegg_modules")}
if(!exists("modules.deMicheli.m")){modules.deMicheli.m <- readRDS("./../saved/modules_deMicheli_m_kegg_modules")}
if(!exists("modules.dellOrso.m")){modules.dellOrso.m <- readRDS("./../saved/modules_dellOrso_m_kegg_modules")}
...
p.primary <- ggplot(df.pathways.primary.separated, aes(x = as.factor(Scorpius_module), y = frequency)) +
  ...
  ggtitle("Frequency of KEGG pathways in different trajectory modules",
          subtitle = "Primary dataset") +
  theme(legend.position="right") +
  geom_text(aes(label=ifelse(frequency>=4, KEGG_pathway, "")), position = position_jitter(width=1, height = 1))
