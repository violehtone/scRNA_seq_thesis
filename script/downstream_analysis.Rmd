...
for(sample in unique(denoised.sce$sample)) {
  ...
  pheatmap(logFCs, breaks=seq(-5, 5, length.out=101), main = paste(sample, " marker genes"), fontsize_row = 5)
}
...
for(sample in unique(denoised.sce.cc$sample)) {
  ...
  # Get marker effects and plot on a heatmap
  logFCs <- getMarkerEffects(markers.interesting)
  pheatmap(logFCs, breaks=seq(-5, 5, length.out=101), main = paste(sample, " (cell cycle corrected) marker genes"), fontsize_row = 5)
}
