...
# Create a new sce object and initialize the module variable
denoised.sce.m <- denoised.sce
rowData(denoised.sce.m)$module <- ""

# Add module information to the rowData of sce object
for(i in 1:nrow(df.mouse.modules)) {
  # Split the gene names into a list
  genes <- unlist(lapply(strsplit(df.mouse.modules[i,]$geneName, split = ",")[[1]], trimws))
  useful_genes <- genes[genes %in% rownames(denoised.sce.m)]
  for(gene in useful_genes) {
    if(rowData(denoised.sce.m)[gene, ]$module == "") {
      # If no module found, set a new module
      rowData(denoised.sce.m)[gene, ]$module <- df.mouse.modules[i,]$module
    }else{
      # If module information already available for the gene, extend the list
      existing_module <- rowData(denoised.sce.m)[gene, ]$module
      rowData(denoised.sce.m)[gene, ]$module <- paste(existing_module, df.mouse.modules[i,]$module, sep = ", ")
      }
    }
}
...
# Create a new sce object and initialize the module variable
denoised.sce.deMicheli.m <- denoised.sce
...
for(i in 1:nrow(df.mouse.modules)) {
  ...
  for(gene in useful_genes) {
    if(rowData(denoised.sce.deMicheli.m)[gene, ]$module == "") {
      # If no module found, set a new module
      rowData(denoised.sce.deMicheli.m)[gene, ]$module <- df.mouse.modules[i,]$module
    }else{
      # If module information already available for the gene, extend the list
      existing_module <- rowData(denoised.sce.deMicheli.m)[gene, ]$module
      rowData(denoised.sce.deMicheli.m)[gene, ]$module <- paste(existing_module, df.mouse.modules[i,]$module, sep = ", ")
      }
    }
}
...
# Create a new sce object and initialize the module variable
denoised.sce.dellOrso.m <- denoised.sce
...
for(i in 1:nrow(df.mouse.modules)) {
  ...
  for(gene in useful_genes) {
    if(rowData(denoised.sce.dellOrso.m)[gene, ]$module == "") {
      # If no module found, set a new module
      rowData(denoised.sce.dellOrso.m)[gene, ]$module <- df.mouse.modules[i,]$module
    }else{
      # If module information already available for the gene, extend the list
      existing_module <- rowData(denoised.sce.dellOrso.m)[gene, ]$module
      rowData(denoised.sce.dellOrso.m)[gene, ]$module <- paste(existing_module, df.mouse.modules[i,]$module, sep = ", ")
      }
    }
}
...
modules.primary.m  <- readRDS("./../saved/modules_primary_m_kegg_modules")
modules.deMicheli.m  <- readRDS("./../saved/modules_deMicheli_m_kegg_modules")
modules.dellOrso.m <- readRDS("./../saved/modules_dellOrso_m_kegg_modules")
...
# Make plots
p.primary <- ggplot(df.modules.primary.separated, aes(x = Scorpius_module, y = frequency)) +
...
p.deMicheli <- ggplot(df.modules.deMicheli.separated, aes(x = Scorpius_module, y = frequency)) +
  geom_bar(aes(fill = KEGG_module), stat = "identity", position = position_dodge(1)) +
  ggtitle("Frequency of KEGG modules in different trajectory modules",
...
p.dellOrso <- ggplot(df.modules.dellOrso.separated, aes(x = Scorpius_module, y = frequency)) +
  geom_bar(aes(fill = KEGG_module), stat = "identity", position = position_dodge(1)) +
  ggtitle("Frequency of KEGG modules in different trajectory modules",
