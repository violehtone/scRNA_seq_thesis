...
```{r load-objects}
denoised.sce.deMicheli <- readRDS("./../saved/denoised_sce_deMicheli_clust")
denoised.sce.dellOrso <- readRDS("./../saved/denoised_sce_dellOrso_clust")
```
...
# plot by cluster
draw_trajectory_plot(space = space.deMicheli, 
                     progression_group = as.factor(denoised.sce.deMicheli$cluster),
                     path = traj.deMicheli$path,
                     contour = TRUE)
```
...
group_name.dellOrso <- as.factor(denoised.sce.dellOrso$sample)
...
# plot by cluster
draw_trajectory_plot(space = space.dellOrso, 
                     progression_group = as.factor(denoised.sce.dellOrso$cluster),
                     path = traj.dellOrso$path,
                     contour = TRUE)
```
